...
namespace ModernGUI.Controls
{
    public class RadioButton : System.Windows.Forms.RadioButton, IControl
    {
        ...
        {
            var width = _boxOffset + 20 + (int)CreateGraphics().MeasureString(Text, SkinManager.OpenSans_MEDIUM_10).Width;
        ...
        protected override void OnPaint(PaintEventArgs pevent)
        {
            ...
            if (Ripple && _rippleAnimationManager.IsAnimating())
            {
                for (var i = 0; i < _rippleAnimationManager.GetAnimationCount(); i++)
                {
                    ...
                    using (var path = ModernGUI.Shared.Drawing.CreateRoundRect(animationSource.X - rippleSize / 2, animationSource.Y - rippleSize / 2, rippleSize, rippleSize, rippleSize / 2))
                    {
            ...
            Color uncheckedColor = ModernGUI.Shared.Drawing.BlendColor(Parent.BackColor, Enabled ? SkinManager.GetCheckboxOffColor() : SkinManager.GetCheckBoxOffDisabledColor(), backgroundAlpha);

            using (var path = ModernGUI.Shared.Drawing.CreateRoundRect(_boxOffset, _boxOffset, RADIOBUTTON_SIZE, RADIOBUTTON_SIZE, 9f))
            {
            ...
            if (Checked)
            {
                using (var path = ModernGUI.Shared.Drawing.CreateRoundRect(RADIOBUTTON_CENTER - animationSizeHalf, RADIOBUTTON_CENTER - animationSizeHalf, animationSize, animationSize, 4f))
                {
                    g.FillPath(brush, path);
                }
            }
            SizeF stringSize = g.MeasureString(Text, SkinManager.OpenSans_MEDIUM_10);
            g.DrawString(Text, SkinManager.OpenSans_MEDIUM_10, Enabled ? SkinManager.GetPrimaryTextBrush() : SkinManager.GetDisabledOrHintBrush(), _boxOffset + 22, Height / 2 - stringSize.Height / 2);
